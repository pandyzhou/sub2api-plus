...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	chatGPTRegisterService := service.NewChatGPTRegisterService(settingRepository, accountRepository)
	chatGPTAccountHandler := admin.NewChatGPTAccountHandler(accountRepository, settingRepository, chatGPTRegisterService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, chatGPTAccountHandler)
	...
	imageSessionService := service.NewImageSessionService(settingRepository)
	chatGPTAccountPoolService := service.NewChatGPTAccountPoolService(accountRepository, settingRepository)
	chatgptImageStorage := service.NewChatGPTImageStorage(configConfig.Pricing.DataDir, "")
	chatgptImageService := service.NewChatGPTImageService(chatGPTAccountPoolService, chatgptImageStorage, configConfig.ExternalImage.Proxy)
	chatgptImageService.SetTokenProvider(openAITokenProvider)
	userImageHandler := handler.NewUserImageHandler(openAIGatewayService, apiKeyService, imageSessionService, chatgptImageService)
